	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(role.equals("OwnerInfo"))
		{
			...
			try
			{
				...
				ResultSet rsLogin;
				rsLogin=stRegister.executeQuery("select * from ownerregistration where Uemail='" +username+ "' and Upassword='"+Password+"'");
				if(rsLogin.next())
				{
		...
		else if(role.equals("UserInfo"))
		{
			...
			try
			{
				...
				if(rsLogin.next())
				{
					session.setAttribute("uname", rsLogin.getString(2));
					session.setAttribute("uemailid", username);
		...
		if(role.contains("Investigator"))
		{
			...
			{
				session.setAttribute("Investigatorname", "Investigator");
				RequestDispatcher rd = request.getRequestDispatcher("/InverstigatorHome.jsp");
				rd.include(request, response); 
			}
			else
			{
		...
		{
			if(username.equals("tpa@gmail.com")&&Password.equals("tpa@123"))
			{
				session.setAttribute("tpaname", "TPA");
				RequestDispatcher rd = request.getRequestDispatcher("/TPAHome.jsp");
				rd.include(request, response); 
			}
			else
			{
