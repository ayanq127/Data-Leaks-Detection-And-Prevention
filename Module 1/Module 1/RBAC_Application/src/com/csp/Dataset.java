...
public class Dataset {
     ...
     */
    public Dataset(int initRows, int initColumns) {
        rows = initRows;
        columns = initColumns;
        data = new byte [rows] [];
     ...
     */
    public static Dataset identity(int size) {
        Dataset result = new Dataset(size, size);
    ...
    public boolean equals(Object other) {
        ...
        for (int r = 0; r < rows; r++) {
            if (!data[r].equals(((Dataset)other).data[r])) {
                return false;
            }
        }
        return true;
     ...
     */
    public Dataset augment(Dataset right) {
        if (rows != right.rows) {
            throw new IllegalArgumentException("Matrices don't have the same number of rows");
        }
        Dataset result = new Dataset(rows, columns + right.columns);
        for (int r = 0; r < rows; r++) {
            for (int c = 0; c < columns; c++) {
     ...
     */
    public Dataset submatrix(int rmin, int cmin, int rmax, int cmax) {
        Dataset result = new Dataset(rmax - rmin, cmax - cmin);
    ...
    public Dataset invert() {
        ...
        Dataset work = augment(identity(rows));
    ...
    private void gaussianElimination() {
        ...
        for (int r = 0; r < rows; r++) {
            ...
            // Scale to 1.
            if (data[r][r] != (byte) 1) {
                byte scale = LogData.divide((byte) 1, data[r][r]);
                for (int c = 0; c < columns; c++) {
                    data[r][c] = LogData.multiply(data[r][c], scale);
                }
            }
            ...
            for (int rowBelow = r + 1; rowBelow < rows; rowBelow++) {
                if (data[rowBelow][r] != (byte) 0) {
                    ...
                    for (int c = 0; c < columns; c++) {
                        data[rowBelow][c] ^= LogData.multiply(scale, data[r][c]);
                    }
                }
            }
        }
        ...
        for (int d = 0; d < rows; d++) {
            for (int rowAbove = 0; rowAbove < d; rowAbove++) {
                if (data[rowAbove][d] != (byte) 0) {
                    ...
                    for (int c = 0; c < columns; c++) {
                        data[rowAbove][c] ^= LogData.multiply(scale, data[d][c]);
                    }
